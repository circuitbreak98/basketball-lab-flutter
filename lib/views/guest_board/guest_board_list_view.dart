import 'package:flutter/material.dart';
import '../../constants/app_constants.dart';
import '../../models/guest_board_model.dart';
import '../../repositories/guest_board_repository.dart';
import '../base/base_board_list_view.dart';
import 'guest_board_detail_view.dart';
import 'guest_board_write_view.dart';

class GuestBoardListView extends BaseBoardListView<GuestBoardModel> {
  const GuestBoardListView({super.key});

  @override
  BaseBoardListViewState<GuestBoardModel> createState() => _GuestBoardListViewState();
}

class _GuestBoardListViewState extends BaseBoardListViewState<GuestBoardModel> {
  final GuestBoardRepository _repository = GuestBoardRepository();

  @override
  GuestBoardRepository get repository => _repository;

  @override
  String get boardTitle => AppConstants.guestBoardName;

  @override
  Widget Function(GuestBoardModel) get createDetailView => 
      (post) => GuestBoardDetailView(post: post);

  @override
  Widget createWriteView({required VoidCallback onPostCreated}) => 
      GuestBoardWriteView(onPostCreated: onPostCreated);
} 